...
// raw 0-31 value written into a FrameBuffer5 gets snapped to the nearest of
...
// stable.
//
// kMaxRaw=31 (5 bit-planes) rather than 15 (4 planes) — see
// DisplayDriver.cpp's kPlaneCount comment for the refresh-rate trade-off
// this represents. Global brightness dimming beyond these 32 discrete
// levels is extended via temporal dithering in DisplayDriver, not more
...
constexpr uint8_t kMaxRaw = 31;
constexpr uint8_t kLevels[] = {
...
// Snaps an arbitrary 0-kMaxRaw value to the nearest level actually present
// in the palette. This is the single mechanism both DisplayDriver's
// scan-plane quantization and any effect wanting a procedural shade
// (gradients, fades, growth animations) should go through, so behavior
// automatically tracks whatever palette is configured above instead of
// hardcoding level indices.
...
inline uint8_t shade(uint32_t numerator, uint32_t denominator) {
  ...
  uint32_t raw = (static_cast<uint32_t>(kMaxRaw) * numerator) / denominator;
  if (raw > kMaxRaw) {
    raw = kMaxRaw;
  }
